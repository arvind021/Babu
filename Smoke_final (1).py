...
async def safe_edit(msg, text, parse_mode=None, reply_markup=None):
    ...
    try:
        await safe_edit(msg, text, parse_mode=parse_mode, reply_markup=reply_markup)
...
def format_result(d, rem_after):
    ...
    if d.get("is_restricted"):
        restricted_block = f"\n⚠️ Restricted » <i>{hv(d.get('restriction_reason','Yes'))}</i>\n"
    ...
    return (
        ...
        f"│  Status     » {hv(d.get('status','—'))}\n"
        f"│  DC ID      » {hv(d.get('dc_id'))}\n"
        f"{common}"
