...
def approve_kb(user_id, uname):
    # Predefined limits for quick approval
    limit_btns = [
        InlineKeyboardButton(f"✅ {l}", callback_data=f"approve_{user_id}_{l}")
...
async def perform_lookup(update: Update, ctx: ContextTypes.DEFAULT_TYPE, query: str):
    ...
    try:
        # Animated progress while fetching
        ...
        await msg.edit_text("❌ *Network Error!*\nAPI slow hai ya down hai. Thodi der baad try karo.", parse_mode=ParseMode.MARKDOWN, reply_markup=back_kb())
...
async def perform_phone_lookup(update: Update, ctx: ContextTypes.DEFAULT_TYPE, number: str):
    ...
    try:
        ...
        await msg.edit_text("❌ *Network Error!* API slow hai. Baad me try karo.", parse_mode=ParseMode.MARKDOWN, reply_markup=back_kb())
    except Exception as e:
        logger.error(e)
        await msg.edit_text(f"❌ *Error:* `{e}`", parse_mode=ParseMode.MARKDOWN, reply_markup=back_kb())
