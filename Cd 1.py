...
FREE_USES      = 2
PHONE_FREE     = 2
GROUP_ID       = int(os.getenv("GROUP_ID", "-1001234567890"))  # Log group ID
GROUP_LINK     = os.getenv("GROUP_LINK", "https://t.me/yourchannel")  # Group link
...
async def safe_edit(msg, text: str, reply_markup=None, parse_mode=HTML):
    ...
    try:
        ...
        elif any(x in err for x in ("can't parse", "bad request", "invalid", "entities")):
            try:
                await msg.edit_text(strip_html(text), parse_mode=None, reply_markup=reply_markup)
            except Exception:
                try:
                    await msg.reply_text(strip_html(text), parse_mode=None, reply_markup=reply_markup)
                except Exception as e2:
...
def main_menu_kb(user_id=None):
    kb = [
        ...
        [InlineKeyboardButton("🆓 Free Lookup — Join Group", url=GROUP_LINK)],
    ]
    if user_id == OWNER_ID:
...
async def perform_lookup(update: Update, ctx: ContextTypes.DEFAULT_TYPE, query: str):
    ...
    if not allowed:
        ...
        if u and u["status"] == "pending":
            ...
            text = (
                "🚫 <b>Free Limit Khatam!</b>\n\n"
                f"Tumhare <code>{FREE_USES}</code> free lookups use ho gaye.\n\n"
                "💎 <b>Premium lo aur 200 lookups karo!</b>\n"
                "┌ Price » <b>₹499/month</b>\n"
                "└ Limit » <b>200 requests</b>"
            )
    ...
    try:
        ...
        clean = {k: v for k, v in data.items() if k not in ("credit","owner","admin","help_group","note","your_usage","key_name","developer")}
        clean["made_by"] = "@l_Smoke_ll"
        clean["developer"] = "@l_Smoke_ll"
...
async def perform_phone_lookup(update: Update, ctx: ContextTypes.DEFAULT_TYPE, number: str):
    ...
    try:
        ...
        # Send raw JSON data
        import json as _json
        clean_p = {k: v for k, v in data.items() if k not in ("credit","owner","admin","help_group","note","your_usage","key_name","developer")}
        clean_p["made_by"] = "@l_Smoke_ll"
        clean_p["developer"] = "@l_Smoke_ll"
    ...
    if data == "buy_premium":
        ...
        try:
            await ctx.bot.send_message(
                ...
                f"👤 User: <a href=\'tg://user?id={uid}\'>{update.effective_user.full_name}</a>\n"
                f"🆔 ID: <code>{uid}</code>\n"
                f"📛 Username: @{uname}\n\n"
                f"💰 Plan: <b>₹499/month</b>\n"
                f"📦 Limit: <b>200 requests</b>\n\n"
                f"Payment confirm hone pe approve karo:",
                parse_mode=HTML,
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("✅ Approve 200", callback_data=f"approve_{uid}_200")],
                    [InlineKeyboardButton("💬 Message User", url=f"tg://user?id={uid}")],
        ...
        await q.message.edit_text(
            ...
            "┌ Plan    » <b>₹499/month</b>\n"
            "└ Limit   » <b>200 requests</b>\n\n"
            "Payment karo aur owner se confirm karwao:\n"
            f"<a href=\'https://t.me/{OWNER_USERNAME}\'>@{OWNER_USERNAME}</a>\n\n"
            "✅ Approve hone pe notification aayega!",
            parse_mode=HTML,
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("💬 Owner se Baat Karo", url=f"https://t.me/{OWNER_USERNAME}")],
                [InlineKeyboardButton("🏠 Main Menu", callback_data="main_menu")],
            ])
        )
        return
...
# ══════════════════════════════════════════════════════════════════
#  👥  GROUP COMMANDS — Unlimited, no limit
# ══════════════════════════════════════════════════════════════════
async def group_user_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Group me /user @username ya /user 123456789"""
    if update.effective_chat.type not in ("group", "supergroup"):
        await update.message.reply_text("❌ Ye command sirf group me kaam karta hai!")
        return
    if update.effective_chat.id != GROUP_ID:
        await update.message.reply_text(
            f"❌ <b>Ye bot sirf official group me kaam karta hai!</b>\n\n"
            f"Join karo: {GROUP_LINK}",
            parse_mode=HTML
        )
        return
    ...
    if not ctx.args:
        await update.message.reply_text(
            "⚠️ Usage: <code>/user @username</code> ya <code>/user 123456789</code>",
            parse_mode=HTML
        )
        return

    query = ctx.args[0]
    await group_perform_lookup(update, ctx, query)

async def group_info_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Group me /info 9876543210"""
    if update.effective_chat.type not in ("group", "supergroup"):
        await update.message.reply_text("❌ Ye command sirf group me kaam karta hai!")
        return
    if update.effective_chat.id != GROUP_ID:
    ...
    if not ctx.args:
        await update.message.reply_text(
            "⚠️ Usage: <code>/info 9876543210</code>",
            parse_mode=HTML
        )
        return

    number = ctx.args[0]
    await group_perform_phone_lookup(update, ctx, number)

async def group_perform_lookup(update: Update, ctx: ContextTypes.DEFAULT_TYPE, query: str):
    """Group lookup — no limit"""
    msg = await update.message.reply_text(
        "⏳ <b>Searching...</b>\n<code>▰▰▰▰▰▱▱▱▱▱ 50%</code>", parse_mode=HTML
    )
    try:
        data = await asyncio.wait_for(fetch_info(query), timeout=60)
        ...
        if not data or data.get("success") == False or "error" in data:
            err = data.get("message") or data.get("error") or "Unknown error"
            await msg.edit_text(f"❌ <b>Error:</b> <code>{he(str(err))}</code>", parse_mode=HTML)
            return

        await msg.edit_text("⏳ <b>Searching...</b>\n<code>▰▰▰▰▰▰▰▰▰▰ 100%</code>", parse_mode=HTML)

        text = format_result(data, 999)
        kb   = result_kb(data.get("username"))
        pic  = data.get("profile_pic")
        ...
        if pic and str(pic).startswith("http"):
            ...
            caption = text[:1020] + "…" if len(text) > 1024 else text
            try:
                await update.message.reply_photo(pic, caption=caption, parse_mode=HTML, reply_markup=kb)
            except Exception:
        ...
        chunks = [json_text[i:i+4000] for i in range(0, len(json_text), 4000)]
        for chunk in chunks:
            await update.message.reply_text("<pre><code>" + chunk + "</code></pre>", parse_mode=HTML)

    except asyncio.TimeoutError:
        await msg.edit_text("❌ <b>Timeout!</b> API ne reply nahi kiya.", parse_mode=HTML)
    except Exception as e:
        logger.exception("group_perform_lookup error")
        await msg.edit_text(f"❌ <b>Error:</b> <code>{he(str(e))}</code>", parse_mode=HTML)

async def group_perform_phone_lookup(update: Update, ctx: ContextTypes.DEFAULT_TYPE, number: str):
    """Group phone lookup — no limit"""
    number = number.strip().replace(" ", "").replace("-", "")
    if not number.lstrip("+").isdigit():
        await update.message.reply_text("❌ Sahi number daalo! Example: <code>9876543210</code>", parse_mode=HTML)
        return
    ...
    try:
        ...
        text = format_phone_result(data, number, 999)
        await msg.edit_text(text, parse_mode=HTML, reply_markup=phone_result_kb())
        ...
        logger.exception("group_perform_phone_lookup error")
        await msg.edit_text(f"❌ <b>Error:</b> <code>{he(str(e))}</code>", parse_mode=HTML)

async def private_block(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Block private messages — sirf group me kaam karta hai"""
    if update.effective_user.id == OWNER_ID:
        await smart_message(update, ctx)
        return
    await update.message.reply_text(
        "❌ <b>Bot sirf group me kaam karta hai!</b>\n\n"
        "Group me /user ya /info command use karo.",
        parse_mode=HTML
    )
...
def main():
    ...
    # ── Owner private handlers ──
    owner_filter = filters.ChatType.PRIVATE & filters.User(OWNER_ID)
    app.add_handler(CommandHandler("start",  start,      filters=filters.ChatType.PRIVATE))
    app.add_handler(CommandHandler("lookup", lookup_cmd, filters=owner_filter))
    app.add_handler(CallbackQueryHandler(button_handler))
    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND & filters.ChatType.PRIVATE, private_block))

    # ── Group handlers — unlimited, free ──
    app.add_handler(CommandHandler("user", group_user_cmd, filters=filters.ChatType.GROUPS))
    app.add_handler(CommandHandler("info", group_info_cmd, filters=filters.ChatType.GROUPS))
