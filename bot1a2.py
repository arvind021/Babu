...
SUPPORT_GROUP  = "https://t.me/+6JT140NC2VtkODk1"   # ← apna link dalo
...
async def upsert_user(u):
    ...
    # Remove fields that are in $set from $setOnInsert to avoid conflict
...
def main_menu_kb(user_id=None, user_is_admin=False):
    """Pass user_is_admin=True if user is admin (checked async before calling)"""
    kb = [
...
async def start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # ── Handle referral link: /start ref_XXXXX ──
    args = ctx.args
    if args and args[0].startswith("ref_"):
        try:
            ...
            if bonus:
                # Notify referrer
                try:
                    ref_u = await get_user(referrer_id)
                    ref_name = he((ref_u or {}).get("full_name") or "User")
...
async def button_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # ══════════════════════════════════════════════════
        ...
        if needed == 0:
            next_line = f"✅ Bonus ready! ({count % REFERRAL_PER_BONUS}/{REFERRAL_PER_BONUS})"
        else:
            next_line = f"{bar}  {needed} more needed"

        await edit(
            f"👥 <b>Referral Program</b>\n━━━━━━━━━━━━━━━━━━\n\n"
            f"Har <b>3 joins</b> pe <b>+1 free search</b> milega!\n"
            f"6 joins = 2 free  |  9 joins = 3 free  ...aur badhta rahe\n\n"
            f"📊 <b>Tumhara Stats</b>\n"
            ...
            InlineKeyboardMarkup([
                [InlineKeyboardButton("🔗 Share Link", url=f"https://t.me/share/url?url={ref_link}&text=Is%20bot%20ko%20try%20karo%21")],
                [InlineKeyboardButton("🏠 Main Menu", callback_data="main_menu")],
            ])
        )
