...
async def safe_edit(msg, text: str, reply_markup=None, parse_mode=HTML):
    async def _edit(t, pm):
        await msg.edit_text(t, parse_mode=pm, reply_markup=reply_markup)
    try:
        await safe_send(_edit, text, parse_mode=parse_mode)
    except BadRequest as e:
        err = str(e).lower()
        if "message to edit not found" in err:
            try:
                await safe_send(msg.reply_text, text, reply_markup=reply_markup, parse_mode=parse_mode)
            except Exception as e2:
...
def format_phone_result(d: dict, number: str, rem: int) -> str:
    name     = d.get("name") or d.get("full_name") or d.get("subscriber") or "—"
    country  = d.get("country") or d.get("country_name") or "—"
    carrier  = d.get("carrier") or d.get("operator") or d.get("provider") or "—"
    region   = d.get("region") or d.get("state") or d.get("city") or "—"
    ltype    = d.get("line_type") or d.get("type") or d.get("connection_type") or "—"
    timezone = d.get("timezone") or d.get("time_zone") or ""
    lines = (
        f"📱 <b>{he(number)}</b>\n\n"
        f"👤 Name    : <b>{hv(name)}</b>\n"
        f"🌍 Country : {hv(country)}\n"
        f"📡 Carrier : {hv(carrier)}\n"
        f"📍 Region  : {hv(region)}\n"
        f"🔌 Type    : {hv(ltype)}\n"
    )
    if timezone:
        lines += f"🕐 Timezone: {hv(timezone)}\n"
    lines += (
        f"\n🔢 Remaining : <code>{rem}</code> lookups\n"
        f"✦ <b>Made by @{OWNER_USERNAME}</b>"
    )
